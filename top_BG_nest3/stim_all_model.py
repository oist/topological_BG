...
def main():
    ...
    if sim_regions['BG']:
        ...
        # combined MSN recorder for whole-population rate (d1 + d2 together)
        msn_params = {"record_to": "ascii", "label": "MSN", "start": float(sim_params['start_time_sp'])}
    ...
    if sim_model_on=='resting_state':
        ...
        if sim_regions['BG']:
          for layer_name in bg_layers.keys():
            n_neurons = nest_routine.count_layer(bg_layers[layer_name])
            rate = nest_routine.average_fr(detectors[layer_name], sim_params['simDuration']-sim_params['start_time_sp'], n_neurons)
            act = nest_routine.instantaneous_fr(detectors[layer_name], sim_params['start_time_sp'], sim_params['simDuration'], n_neurons, float(sim_params['dt']))
        ...
        for ni,i in enumerate(CSN_columns_gids_subset): #where those 200 neurons are connected ?
            for tgt_name, gid_pos in [('MSN_d1', gid_pos_MSN_d1), ('MSN_d2', gid_pos_MSN_d2)]:
                conns = nest.GetConnections(source=nest.NodeCollection(sorted([gid[0] for gid in i])),
                                            target=nest.NodeCollection(sorted(list(gid_pos[:,0].astype(int)))))
                if len(conns) > 0:
                    d = conns.get(['source','target','weight'])
                    np.savetxt('./log/'+'CSN_c'+str(ni)+'_to_'+tgt_name+'.txt',
                               np.column_stack([d['source'], d['target'], d['weight']]))
        for ni,i in enumerate(PTN_columns_gids_subset): #where those 200 neurons are connected ?
            for tgt_name, gid_pos in [('MSN_d1', gid_pos_MSN_d1), ('MSN_d2', gid_pos_MSN_d2)]:
                ...
                if len(conns) > 0:
                    d = conns.get(['source','target','weight'])
                    np.savetxt('./log/'+'PTN_c'+str(ni)+'_to_'+tgt_name+'.txt',
                               np.column_stack([d['source'], d['target'], d['weight']]))
        ##########################################
        ...
        for i in np.arange(11):  #channel 1
            for j in np.arange(11):   #channel 2
                ...
                sp_MSN = nest_routine.get_events_from_ascii(detectors['MSN'])
                ...
                sp_MSN_d1 = nest_routine.get_events_from_ascii(detectors['MSN_d1'])
                ...
                sp_MSN_d2 = nest_routine.get_events_from_ascii(detectors['MSN_d2'])
                ...
                sp_GPi = nest_routine.get_events_from_ascii(detectors['GPi'])
                ...
                sp_PTN = nest_routine.get_events_from_ascii(detectors['PTN'])
                ...
                ####### For MSN (get mean firing rate )####################################
                sp_CSN = nest_routine.get_events_from_ascii(detectors['CSN'])
        ...
        if bg_params['plastic_syn']:
            ...
            PTN_sd_1 = nest.Create("spike_recorder", params={'label': 'PTN_300_ch_1', "record_to": "ascii"})
            nest.Connect(nest.NodeCollection(sorted([item[0] for item in PTN_columns_gids_subset[channel_1_idx]])), PTN_sd_1)

            PTN_sd_2 = nest.Create("spike_recorder", params={'label': 'PTN_300_ch_2', "record_to": "ascii"})
            nest.Connect(nest.NodeCollection(sorted([item[0] for item in PTN_columns_gids_subset[channel_2_idx]])), PTN_sd_2)

            CSN_sd_1 = nest.Create("spike_recorder", params={'label': 'CSN_300_ch_1', "record_to": "ascii"})
            nest.Connect(nest.NodeCollection(sorted([item[0] for item in CSN_columns_gids_subset[channel_1_idx]])), CSN_sd_1)

            CSN_sd_2 = nest.Create("spike_recorder", params={'label': 'CSN_300_ch_2', "record_to": "ascii"})
            nest.Connect(nest.NodeCollection(sorted([item[0] for item in CSN_columns_gids_subset[channel_2_idx]])), CSN_sd_2)
            ###### ####################################
            ...
            for episode in np.arange(total_episodes):
                if episode > con_ep:  # CS- discrimination phase
                    ...
                    #STDPs for DA dips
                    nest.SetDefaults('syn_d1',{'volume_transmitter':bg_params['vt_d2'],
                    ...
                    nest.SetDefaults('syn_d2',{'volume_transmitter':bg_params['vt_d1'],
                ...
                sp_FSI = nest_routine.get_events_from_ascii(detectors['FSI'])
                ...
                sp_GPe = nest_routine.get_events_from_ascii(detectors['GPe'])
                ...
                sp_STN = nest_routine.get_events_from_ascii(detectors['STN'])
                    ...
                    if current_task[-1] == -1 and MSN_d2_fr[-1][2]<70.0:  #CS- task 
                        if GPi_fr[-1][channel_1_idx] < GPi_fr[-1][channel_2_idx]: #if channel 1 is winner
                            ...
                            if not(modulated): #then modulate.
                                ...
                                my_source = bg_layers['MSN_d2']
                                conn_d1 = nest.GetConnections(source=my_source, target=bg_layers['MSN_d1'])
                                conn_d2 = nest.GetConnections(source=my_source, target=bg_layers['MSN_d2'])
                                old_weight = nest.GetStatus(conn_d1)[0]['weight'] if len(conn_d1) > 0 else nest.GetStatus(conn_d2)[0]['weight']

                                newweight = old_weight*my_gain
                                if len(conn_d1) > 0:
                                    nest.SetStatus(conn_d1, {'weight': newweight})
                                if len(conn_d2) > 0:
                                    nest.SetStatus(conn_d2, {'weight': newweight})

                                my_target_1 = bg_layers['GPe']
